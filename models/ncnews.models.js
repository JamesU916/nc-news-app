...
exports.fetchArticles = (sort_by = "created_at", order = "desc", topic) => {
    ...
    return db.query(queryString, queryParameters).then(({ rows }) => {
        if(rows.length === 0 && topic) {
            ...
            .then(({ rows: topics }) => {
                if(topics.length === 0) {
                    return Promise.reject({status: 404, msg: "404 Not Found - Topic does not exist"})
                }
                return []
            })
        } 
...
exports.fetchUserByUsername = (username) => {
    return db.query(`SELECT users.username, users.name, users.avatar_url FROM users WHERE users.username = $1`, [username])
    .then(({ rows }) => {
        if (rows.length === 0) {
